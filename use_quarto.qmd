...
HTML形式で出力する際の便利な設定をいくつか紹介します。

### 目次（Table of Contents）の追加

長い文書では目次があると便利です。YAMLヘッダーに以下を追加します。
...
- `toc: true`: 目次を表示
- `toc-depth: 3`: 見出しレベル3まで目次に含める
- `toc-location: left`: 目次を左側に配置（`right`や`body`も選択可能）

### コードの折りたたみ

コードを折りたたんで表示することもできます。
...
- `code-fold: true`: コードを折りたたんで表示（クリックで展開可能）
- `code-tools: true`: コードの表示/非表示を切り替えるボタンを追加

### テーマの変更

Quartoには多くの組み込みテーマがあります。
...
利用可能なテーマ: `default`, `cosmo`, `flatly`, `journal`, `lumen`, `paper`, `readable`, `sandstone`, `simplex`, `yeti`など

::: {.callout-tip}
## おすすめテーマ

- `cosmo`: モダンで読みやすい
- `flatly`: シンプルでプロフェッショナル
- `sandstone`: 温かみのあるデザイン（このサイトでも使用）
:::

## PDF出力について

PDF出力には$\LaTeX$環境が必要です。最も簡単な方法は、TinyTeXをインストールすることです。
...
インストール後、Quarto文書のYAMLヘッダーで`format: pdf`を指定します。
...
- `pdf-engine: lualatex`: 日本語対応のLaTeXエンジン
- `documentclass: ltjsarticle`: 日本語用の文書クラス

::: {.callout-caution}
## PDF出力の注意点

PDF出力は環境によって問題が発生することがあります。エラーが出た場合は、まずHTML出力で動作確認することをおすすめします。PDF出力は慣れてから挑戦しても遅くありません。
:::

## 実践例：データ分析レポートを作成する

ここまで学んだことを使って、簡単なデータ分析レポートを作成してみましょう。

以下は`iris`データセットを使った分析レポートの例です。
...
iris |>
  group_by(Species) |>
  summarize(
    平均 = mean(Sepal.Length),
    標準偏差 = sd(Sepal.Length)
  )
```

## 可視化

がく片の長さと幅の関係を品種ごとに可視化します。
...
品種ごとに明確な違いが見られました。特に setosa は他の2品種と
異なる特徴を持っていることが確認できました。
````

この例をコピーして新しいQuarto文書に貼り付け、Renderしてみてください。データの読み込みから可視化まで、一連の流れを含んだレポートが完成します。

## まとめ

Quartoを使うことで、以下のことが簡単にできるようになります。

- コードと結果を含む文書の作成
- HTML、PDF、スライドなど多様な形式での出力
- 再現可能な分析レポートの作成

データ分析の結果を共有する際、Quartoは非常に強力なツールです。ぜひ活用してみてください。

今後のデータ分析の章で学ぶ内容を、Quartoでレポートにまとめることで、より実践的なスキルが身につきます。
